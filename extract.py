...
for package in os.listdir(os.getcwd() + '/data'):
    if '.di3' in package:
        ...
        try:
            ...
            package_res_name = '!RESULT/temp' + package
            ...
            if (os.path.exists(package_res_name + '/que1') and
                ...
                for line in f:
                    document.add_paragraph(line)
                f.close()
                package_res_name=package_res_name.replace("temp","")
                ...
                if os.path.exists(package_res_name + '/res'):
                    ...
                    for line in f:
                        if i==8:
                            line=line.replace("\n","")
                            document.save(package_output_name+"\ТЕСТ НА " + line + " БАЛЛОВ, "+package)
                            document=Document()
                        i = i + 1
                    f.close()
        except:
            ...
            break
# удаление временных папки
   
for package2 in os.listdir(os.getcwd() + '/' + package_output_name):
    if "temp" in package2:
        path = os.path.join(os.getcwd() + '/' + package_output_name, package2)
        shutil.rmtree(path)
